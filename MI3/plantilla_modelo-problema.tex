...
Sin embargo, la viabilidad de estos sistemas de seguridad depende críticamente de la evolución de las técnicas de análisis numérico y de la capacidad de cómputo disponible \citep{odlyzko1995}. El desarrollo histórico de los algoritmos de factorización avanzados, diseñados para romper la barrera del tiempo exponencial mediante aproximaciones heurísticas, ha permitido suavizar drásticamente las curvas de coste temporal. Entre los métodos deterministas no rigurosos más rápidos de la literatura destacan el método de las fracciones continuas o CFRAC, propuesto por \citet{morrison1975}; la criba cuadrática (QS), desarrollada por \citet{pomerance1982}; y la criba general del cuerpo de números (GNFS), consolidada por \citet{lenstra1993}. Comprender con precisión matemática las fronteras asintóticas de estos órdenes de complejidad subexponenciales es fundamental para anticipar la obsolescencia de los estándares criptográficos actuales y garantizar la integridad de las infraestructuras de datos en el futuro.
...
\begin{itemize}
    ...
    \item \textbf{Capítulo 2: Objetivos.} Se establecen de manera concisa las metas metodológicas de la investigación, acotando el alcance del estudio en torno a la fundamentación teórica, la evaluación de métodos clásicos y la caracterización analítica del horizonte subexponencial.
    \item \textbf{Capítulo 3: Enunciado del problema.} Se delimita el marco conceptual de las tareas computacionales propuestas por el ejercicio. En él se plantean formalmente los requerimientos de la demostración del teorema del factor primo, las cuestiones sobre la naturaleza operativa de la división por tentativa y las premisas algebraicas que definen la criba subexponencial.
...
Se presenta de manera formal el método más intuitivo para abordar la primalidad, el cual comprueba la divisibilidad de un entero compuesto $n$ de forma secuencial entre los números primos hasta su límite teórico de $\sqrt{n}$. Con el objetivo de evaluar la viabilidad computacional de este procedimiento, se plantean formalmente las siguientes cuestiones para su posterior resolución analítica:
...
Aunque el algoritmo de división por tentativa presenta ciertas ventajas operativas frente a otros métodos, sus limitaciones estructurales dificultan drásticamente su escalabilidad en escenarios prácticos. A continuación, se detallan las principales ventajas y desventajas de este procedimiento:
...
\begin{enumerate}
    ...
    $$\lim_{k \to \infty} \frac{g(k)}{k} = 0$$
        Este resultado demuestra que el exponente de estos algoritmos de factorización son asintóticamente menores que cualquier exponente lineal, confirmando que la función general $e^{g(x)}$ crece más lento que un tiempo exponencial puro $\mathcal{O}(c^k)$.
    ...
    Por la regla de la jerarquía de infinitos, una función lineal encadenada con un logaritmo, como $k \ln(k)$, domina por completo a cualquier potencia de un logaritmo en el denominador. Por consiguiente, el límite diverge al infinito:
...
\begin{itemize}
    ...
    Al evaluar $n^{\epsilon}$ en la primera función de coste, se aplica la propiedad del logaritmo de una potencia ($\ln n^{\epsilon} = \epsilon \ln n$), lo que permite cancelar la raíz cuadrada de forma exacta:
    $$e^{\sqrt{(\ln n^{\epsilon}) (\ln n^{\epsilon})}} = e^{\sqrt{(\epsilon \ln n)^2}} = e^{\ln(n^{\epsilon})} = n^{\epsilon}$$
    ...
    Al evaluar la función en la segunda expresión de coste ($e^{\sqrt{(\ln \ln n)(\ln \ln n)}} = e^{\ln \ln n} = \ln n$), se aplica primero la propiedad del logaritmo sobre el término sustituido: $\ln(\ln n)^c = c \ln \ln n$. Al introducirlo en la estructura del doble logaritmo, la raíz se simplifica de forma análoga:
    $$e^{\sqrt{\ln(\ln n)^c \cdot \ln(\ln n)^c}} = e^{\sqrt{(c \ln \ln n)^2}} = e^{\ln(\ln n)^c} = (\ln n)^c$$
...
El principal núcleo analítico de este Trabajo ha culminado con la demostración rigurosa, mediante el cálculo de límites al infinito, de la naturaleza de la función de coste $\mathcal{O} \left(e^{\sqrt{(\ln n)(\ln \ln n)}}\right)$. Al trasladar el análisis al dominio de los bits ($k$) y contrastar el comportamiento frente a funciones de orden polinomial ($k^c$) y exponencial puro ($2^{k\epsilon}$), se ha probado formalmente su carácter intermedio o subexponencial. El valor crítico de este logro radica en comprender el comportamiento de los algoritmos avanzados de factorización como CFRAC, QS o GNFS \citep{morrison1975, pomerance1982, lenstra1993}: estos métodos no logran la eficiencia idónea de un tiempo polinomial, pero sí consiguen disminuir la curva de crecimiento temporal de forma drástica, posicionándose como la frontera del límite tecnológico actual para comprometer la seguridad de clave pública \citep{odlyzko1995}.
\vspace{0.3cm}

\noindent \textbf{Consideraciones finales e implicaciones criptográficas} \\
Como balance global del Trabajo, la resolución de las cuestiones planteadas pone de manifiesto que la seguridad digital no es estática, sino que depende directamente de la evolución del análisis numérico asintótico. La aportación de este documento radica en clarificar de forma transparente dichas fronteras de complejidad. Los resultados obtenidos permiten concluir de manera crítica que, mientras no se descubra un algoritmo determinista riguroso de orden polinomial en computación clásica, o mientras la computación cuántica y el célebre algoritmo de \cite{shor1994} no alcancen una escala de despliegue comercial y estabilidad operativa, el equilibrio de la infraestructura criptográfica global seguirá dependiendo de mantener la brecha de eficiencia que separa a la multiplicación de la factorización subexponencial analizada.
